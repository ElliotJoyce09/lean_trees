...
lemma acyclic_graphs_have_a_leaf {V : Type} [Finite V] (G : SimpleGraph V) (nonempty : Nonempty V) (g_is_acyclic : isAcyclic G) (nontrivial : (Fintype.ofFinite G.edgeSet).card > 0 ) : hasLeaf G := by sorry
  ...
  |hi y hy =>
    ...
    have G'_has_y_edges : ((Fintype.ofFinite ↑(G'.edgeSet)).card) = y := by
      ...
      have G'_has_one_less_edge_than_G : ((Fintype.ofFinite ↑(G'.edgeSet)).card) = ((Fintype.ofFinite (G.edgeSet)).card) - 1 := by
        ...
        have G_without_x_leaf_is_G' : (G.edgeSet \ {s(x.1, leaf)}) = (G'.edgeSet) := by
          ...
          -- In order to show our goal, we need to first show that the only element in teh incident set of leaf is the edge (x, leaf)
          have incidentset_eq : G.incidenceSet leaf = {s(x.1, leaf)} := by
            ...
            · by_contra incidentset_notin_sxleaf -- Assume ¬ (G.incidenceSet leaf ⊆ s(↑x, leaf)})
                ...
                · rename_i not_u_is_leaf
                  ...
                  · rename_i not_v_is_leaf
                    ...
                    have u_or_v_is_leaf : (leaf = u ∨ leaf = v) := by
                      ...
                      -- To prove this, we will show using previous results that such an edge is an edge in the parent graph with one of its endpoints being equal to leaf
                  ...
                  · rename_i not_not_v_is_leaf
                    ...
                    -- So we have that u ≠ x. If we show u is still in teh neighbor set of leaf then we will be trivially done
              ...
              · rename_i u_and_v_are_not_leaf
                ...
                -- And so we get our contradiction from teh assumption and the previous result
                exact not_u_or_v_leaf u_or_v_is_leaf
          ...
          have subset : (G.edgeSet \ G.incidenceSet leaf) ⊆ (G'.edgeSet) := by
            ...
            have z_in_Gwithoutleaf_imp_z_in_G' : ∀ z ∈ (G.edgeSet \ (G.incidenceSet leaf)), z ∈ G'.edgeSet := by
              ...
              -- We have from previous results and the lemma deleteEdges to show that (G.deleteEdges (G.incidenceSet leaf)).edgeSet = G.edgeSet \ (G.incidenceSet leaf)
          ...
          have superset : (G'.edgeSet) ⊆ (G.edgeSet \ G.incidenceSet leaf) := by
            ...
            have z_in_G'_imp_z_in_Gwithoutleaf : ∀ z ∈ G'.edgeSet, z ∈ (G.edgeSet \ (G.incidenceSet leaf)) := by
              ...
              -- Now that we have in_Set in the deleteEdges form, by proving (G.deleteEdges (G.incidenceSet leaf)).edgeSet = G.edgeSet \ (G.incidenceSet leaf) the goal can be closed with in_Set
              have Gwithoutleaf_eq_z_in_G' : (G.deleteEdges (G.incidenceSet leaf)).edgeSet = G.edgeSet \ (G.incidenceSet leaf) := by
                exact SimpleGraph.edgeSet_deleteEdges (G.incidenceSet leaf)
    ...
    have G_preconnected : G.Preconnected := by
      ...
      have all_verts_in_G'_reachable : ∀ a b : G'.verts, G.Reachable ↑a ↑b := by
        ...
        -- And so rewriting all the vertices in the subgraph with vertices in teh graph, we can close this goal
      ...
      have a_leaf_reachable : ∀ a : V, G.Reachable a leaf := by
        ...
        -- Now let us introduce a vertex a. Note that we have already done the work in teh case that a = leaf or a = x
        intro a
